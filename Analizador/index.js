import SimpleAnalizadorLexer from "./generated/SimpleAnalizadorLexer.js";
import SimpleAnalizadorParser from "./generated/SimpleAnalizadorParser.js";
import CustomSimpleAnalizadorVisitor from "./CustomSimpleAnalizadorVisitor.js";
...
async function main() {
    ...
    let lexer = new SimpleAnalizadorLexer(inputStream);
    ...
    for (let token of tokens) {
        ...
        const tokenType = SimpleAnalizadorLexer.symbolicNames[token.type] || `UNKNOWN (${token.type})`;
    ...
    lexer = new SimpleAnalizadorLexer(inputStream);
    let tokenStream = new CommonTokenStream(lexer);
    let parser = new SimpleAnalizadorParser(tokenStream);
    let tree = parser.prog();

    // Verificar si se produjeron errores sintácticos
    if (parser.syntaxErrorsCount > 0) {
        console.error("\nSe encontraron errores de sintaxis en la cadena de entrada.");
    } else {
        console.log("\nEntrada válida.");
        ...
        const visitor = new CustomSimpleAnalizadorVisitor();
        visitor.visit(tree);
    }
}
